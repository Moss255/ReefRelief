...
HX_STACK_ARG(manager,"manager")
{
	...
	HX_STACK_LINE(26)
	::flixel::tweens::FlxTweenManager tmp1 = manager;		HX_STACK_VAR(tmp1,"tmp1");
	HX_STACK_LINE(26)
...
::flixel::tweens::misc::VarTween VarTween_obj::tween( Dynamic object,Dynamic properties,Float duration){
	...
	if ((tmp)){
		HX_STACK_LINE(41)
		HX_STACK_DO_THROW(HX_HCSTRING("Cannot tween variables of an object that is null.","\x4e","\x3a","\x13","\x2f"));
	}
	else{
		HX_STACK_LINE(43)
		...
		if ((tmp1)){
			HX_STACK_LINE(45)
			HX_STACK_DO_THROW(HX_HCSTRING("Cannot tween null properties.","\x74","\x80","\x03","\x52"));
		}
	}
	HX_STACK_LINE(49)
	this->_object = object;
...
Void VarTween_obj::initializeVars( ){
{
		...
		HX_STACK_LINE(96)
		{
			...
			while((true)){
				...
				HX_STACK_LINE(98)
				::String tmp6 = p;		HX_STACK_VAR(tmp6,"tmp6");
				HX_STACK_LINE(98)
				...
				if ((tmp8)){
					HX_STACK_LINE(100)
					::String tmp9 = (HX_HCSTRING("The Object does not have the property \"","\x3c","\x1e","\xc1","\xa6") + p);		HX_STACK_VAR(tmp9,"tmp9");
					HX_STACK_LINE(100)
				...
				if ((tmp13)){
					HX_STACK_LINE(107)
					::String tmp14 = (HX_HCSTRING("The property \"","\x06","\xe3","\x80","\xa4") + p);		HX_STACK_VAR(tmp14,"tmp14");
					HX_STACK_LINE(107)
				...
				struct _Function_3_1{
					inline static Dynamic Block( Float &tmp20,Float &tmp15,::String &tmp14){
						HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","flixel/tweens/misc/VarTween.hx",110,0x836bddcb)
						{
